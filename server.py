...
# ── RUTAS: UPLOAD / DELETE PDFs ─────────────────────────────────
@app.post("/api/admin/upload")
def upload_pdf():
    """Sube un PDF y lo asocia a una materia (admin only)."""
    u = current_user()
    if not u:                return jsonify({"error": "No autorizado"}), 401
    if u["role"] != "admin": return jsonify({"error": "Solo administradores"}), 403

    mid  = request.form.get("materia_id")
    tipo = request.form.get("tipo", "material")   # "programa" | "material"
    if not mid:
    ...
    # Carpeta destino
    if tipo == "programa":
        dest_dir = os.path.join(app.root_path, "static", "programas")
        filename  = secure_filename(file.filename)
        file.save(os.path.join(dest_dir, filename))
        # Actualizar materias.json
        data = mat_load()
        for m in data:
            if str(m.get("id")) == str(mid):
                m["archivo_pdf"] = filename
                break
        with open(MAT_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        return jsonify({"message": "Programa subido", "archivo": filename})
    else:
        # Guardar en static/materiales/<CARPETA_MATERIA>/
        data = mat_load()
        materia = next((m for m in data if str(m.get("id")) == str(mid)), None)
        if not materia:
            return jsonify({"error": "Materia no encontrada"}), 404

        carpeta = materia["nombre"].upper().replace(",","").replace(".","")
        dest_dir = os.path.join(app.root_path, "static", "materiales", carpeta)
        os.makedirs(dest_dir, exist_ok=True)
        filename = secure_filename(file.filename)
        file.save(os.path.join(dest_dir, filename))

        # Agregar a materiales de la materia
        entry = {"carpeta": carpeta, "archivo": filename}
        if entry not in materia.get("materiales", []):
            materia.setdefault("materiales", []).append(entry)
        with open(MAT_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        return jsonify({"message": "Material subido", "archivo": filename, "carpeta": carpeta})


@app.delete("/api/admin/materiales/<int:mid>")
def delete_material(mid: int):
    """Elimina un material de una materia (admin only)."""
    u = current_user()
    if not u:                return jsonify({"error": "No autorizado"}), 401
    if u["role"] != "admin": return jsonify({"error": "Solo administradores"}), 403
    ...
    data    = mat_load()
    materia = next((m for m in data if m["id"] == mid), None)
    if not materia:
        return jsonify({"error": "Materia no encontrada"}), 404

    # Quitar del JSON
    materia["materiales"] = [
        e for e in materia.get("materiales", [])
        if not (e.get("archivo") == archivo and e.get("carpeta") == carpeta)
    ]
    with open(MAT_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    ...
    if carpeta and archivo:
        path = os.path.join(app.root_path, "static", "materiales", carpeta, secure_filename(archivo))
        try:
